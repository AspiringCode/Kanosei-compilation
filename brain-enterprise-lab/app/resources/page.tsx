...
const CATEGORY_COLOR: Record<string, string> = {
  budget: '#f59e0b', capacity: '#8b5cf6', compute: '#3b82f6', tools: '#22d3ee', time: '#10b981'
}

const PIE_COLORS = ['#f59e0b', '#8b5cf6', '#3b82f6', '#10b981', '#ec4899', '#22d3ee']
    ...
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Total Spent', value: '$1.14M', sub: '47.5% of budget', color: '#8b5cf6', icon: <TrendingUp size={15} /> },
        ...
        <motion.div {...stagger(5)} className="glass-card p-5">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              ...
              <Tooltip
                contentStyle={{ background: '#0d0d1c', border: '1px solid rgba(139,92,246,0.2)', borderRadius: '8px', fontSize: '11px' }}
          ...
          {RESOURCES.map((res, i) => {
            ...
            return (
              <motion.div key={res.id} {...stagger(i + 8)}
                className={cn('glass-card p-4 relative overflow-hidden transition-all hover:border-[rgba(139,92,246,0.25)]',
        ...
        <motion.div {...stagger(16)} className="glass-card p-5">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={BUDGET_ALLOCATIONS.map(b => ({ dept: b.department, allocated: b.allocated/1000, spent: b.spent/1000 }))}
              ...
              <Tooltip contentStyle={{ background: '#0d0d1c', border: '1px solid rgba(139,92,246,0.2)', borderRadius: '8px', fontSize: '11px' }}
                formatter={(v: number) => [`$${v.toFixed(0)}K`, '']} />
              <Bar dataKey="allocated" fill="rgba(139,92,246,0.25)" name="Allocated" radius={[2, 2, 0, 0]} />
              <Bar dataKey="spent" fill="#8b5cf6" name="Spent" radius={[2, 2, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </motion.div>
